...
import { ArrowLeft, MapPin, Star, Eye, SlidersHorizontal, Loader2 } from 'lucide-react';
...
export default function HotelsPage() {
  ...
  const { data: rawHotels, loading, error, refetch } = useTravelList('hotels', {
    limit: 200,
  ...
  const hotels = useMemo(() => {
    return (rawHotels || []).map((h) => ({
      ...
      name: h.name,
      location: h.city || '',
      image: (h.images && h.images[0]) || HOTEL_FALLBACK,
      amenities: Array.isArray(h.amenities) ? h.amenities : [],
      rating: Math.min(5, Math.max(1, Math.round(Number(h.starRating ?? h.reviewScore ?? 5)))),
  ...
  const filtered = useMemo(() => {
    ...
    if (maxPrice) list = list.filter((h) => h.pricePerNight <= Number(maxPrice));
    if (minRating) list = list.filter((h) => h.rating >= Number(minRating));
    return list;
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 py-6 sm:py-8 md:py-10 max-w-7xl min-w-0">
        ...
        <div className="flex items-center justify-between gap-4 mb-4 lg:hidden">
          ...
          <span className="text-sm text-muted-foreground">
            {loading ? 'Loading…' : `${filtered.length} hotel(s) found`}
          </span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            ...
            <p className="text-muted-foreground text-sm mb-4 hidden lg:block">
              {loading ? 'Loading…' : `${filtered.length} hotel(s) found`}
            </p>
            {loading && (
                      ...
                      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between min-w-0">
                        <div className="min-w-0">
                          <div className="flex items-center gap-1 mb-1.5 flex-wrap">
                            {[...Array(5)].map((_, k) => (
                              <Star key={k} className={`h-4 w-4 shrink-0 ${k < hotel.rating ? 'fill-primary text-primary' : 'text-muted-foreground/30'}`} />
                            ))}
                          ...
                          <p className="text-sm text-muted-foreground line-clamp-2 min-w-0">
                            {hotel.amenities.length ? hotel.amenities.join(' · ') : '—'}
                          </p>
                        </div>
