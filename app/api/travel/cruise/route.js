/**
 * GET /api/travel/cruise?destination=...&minPrice=0&maxPrice=100000
 * &sortBy=price&order=asc&page=1&limit=12
 */
import { searchCruise } from '@/lib/travel';
...
export async function GET(request) {
  try {
    ...
    let data = await searchCruise({ destination });

    if (minPrice != null) data = data.filter((c) => (c.price ?? 0) >= minPrice);
    if (maxPrice != null) data = data.filter((c) => (c.price ?? 0) <= maxPrice);
    ...
    console.error('Cruise API error:', err);
    return error(err.message || 'Cruise search failed', 500);
  }
}
