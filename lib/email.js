  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      ...
      <p style="margin: 24px 0;">
        <a href="${resetUrl}" style="display: inline-block; background: #2563eb; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600;">Reset password</a>
      </p>
...
export async function sendNotificationEmail(to, subject, title, message = '', link = '', ctaText = 'View details') {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      ...
      ${link ? `<p style="margin: 24px 0;"><a href="${href}" style="display: inline-block; background: #2563eb; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600;">${String(ctaText).replace(/</g, '')}</a></p>` : ''}
...
export async function sendOfferEmail(to, subject, title, message = '', link = '/offers', ctaText = 'View offer') {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      <div style="padding: 24px 0; border-bottom: 1px solid #eee;">
        <span style="font-size: 12px; font-weight: 600; color: #2563eb; text-transform: uppercase;">Special offer</span>
        <h1 style="margin: 8px 0 0; font-size: 20px; font-weight: 600;">${String(title).replace(/</g, '')}</h1>
      </div>
      ${message ? `<p style="margin: 24px 0; line-height: 1.5;">${String(message).replace(/</g, '&lt;')}</p>` : ''}
      <p style="margin: 24px 0;"><a href="${href}" style="display: inline-block; background: #2563eb; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600;">${String(ctaText).replace(/</g, '')}</a></p>
      <p style="margin: 24px 0 0; font-size: 13px; color: #999;">— The ${BRAND_NAME} Team</p>
    </div>
  `;
  return sendEmail(to, subject, html, text);
...
export async function sendBookingConfirmationEmail(userEmail, userName, bookingId, booking = {}) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      ...
      <p style="margin: 24px 0;"><a href="${myBookingsUrl}" style="display: inline-block; background: #2563eb; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: 600;">View my bookings</a></p>
      <p style="margin: 24px 0 0; font-size: 13px; color: #999;">— The ${BRAND_NAME} Team</p>
    </div>
  `;
  return sendEmail(userEmail, subject, html, text);
